...
module corner(rod_r=4, rod_center_offset=8, rod_origin_offset=20) {
  ...
  difference() {
    ...
    translate([22.5, -22.5, -50]) union() { 
      translate([0, 0, 0]) rotate([0, 0, 45]) {nema17_simple();}
      translate([0, 0, 35.9]) hull() {
        translate([0, 0, 0]) cylinder(r=6, h=50);
        translate([160, 0, 0]) cylinder(r=6, h=50);
        translate([0, -160, 0]) cylinder(r=6, h=50);
      }
    }
  }
}
...
module x_axis_motion(rod_r=4, rod_center_offset=8) {
  hole_r = rod_r * 1.04;
  wall_r = hole_r * 2.5;
  motor = 42;
  motor_frame = 50;
